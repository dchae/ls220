...
function dfs(grid, row, col) {
  ...
  for (let offset = -1; offset < 2; offset += 2) {
    treeCount += dfs(grid, row + offset, col);
    treeCount += dfs(grid, row, col + offset);
  }
...
function largestForestArea(grid) {
  ...
  for (let row = 0; row < grid.length; row++) {
    for (let col = 0; col < grid[0].length; col++) {
      // if cur cell is an unseen tree, explore the forest
      if (grid[row][col] === "T") {
        let newForestSize = dfs(grid, row, col);
        best = Math.max(best, newForestSize);
      }
    }
  }
