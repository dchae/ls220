...
2. iterate through actions
  - if action is a number, add it to scoreboard
  - if action is +, sum the last two numbers and push
  - if action is *, multiply the last number by two and push
  - if action is -, pop
3. sum scoreboard and return (default 0)
*/
...
function nexusSurge(actions) {
  ...
  for (let action of actions) {
    switch (action) {
      case "+":
        scoreboard.push(scoreboard.at(-2) + scoreboard.at(-1));
        break;
      case "*":
        scoreboard.push(2 * scoreboard.at(-1));
        break;
      case "-":
        scoreboard.pop();
        break;
      default:
        scoreboard.push(+action);
    }
  }

  return scoreboard.reduce((acc, x) => acc + x, 0);
}
